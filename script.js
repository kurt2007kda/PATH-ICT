...
function renderJobs(jobs) {
    ...
    if (jobs.length === 0) {
        jobResults.innerHTML = '<li>No results found.</li>';
        return;
    }
    ...
    jobs.forEach(job => {
        const listItem = document.createElement('li');
        listItem.innerHTML = `
            ${job.link ? `<a href="${job.link}" target="_blank"><strong>${job.title}</strong></a>` : `<strong>${job.title}</strong>`}
            ${job.description ? `<p><strong>Description:</strong> ${job.description}</p>` : ''}
            ${job.additionalDescription ? `<p><strong>Details:</strong> ${job.additionalDescription}</p>` : ''}
            ${job.skill ? `<p><strong>Required Skills:</strong> ${job.skill}</p>` : ''}
        `;
        jobResults.appendChild(listItem);
    });
}
...
jobSearchForm.addEventListener('submit', (event) => {
    ...
    if (query) {
        ...
        // Mock job search (case-insensitive)
